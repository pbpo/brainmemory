...
# -*- coding: utf-8 -*-
"""
HTNR Memory — V56-ANN+ (EmbeddingGemma edition)
...
- 머지 후보도 ANN 우선, 실패 시 소형 서브매트릭스 폴백
- MMR 문장 임베딩 사전 계산 및 재사용
- OOC GPU 스트리밍(matvec) 스트림/버퍼 캐시
- HNSW capacity slack & resize
- V55X 스타일 옵션 유지
...
class PageHinkley:
    ...
    def _update_moments(self, x: float) -> Tuple[float, float]:
        ...
        if self.alpha >= 1.0 or self.count == 1:
            ...
            diff = x - prev_mean
            self.var_m2 = a * (self.var_m2 + (1.0 - a) * diff * diff)
            std = math.sqrt(max(0.0, self.var_m2))
            return self.mean, std
...
# ------------------------------------------------------------
# EVT tail fitter (GPD via Pickands estimator)
# ------------------------------------------------------------
@dataclass
class EVTConfig:
    q0: float = 0.90
...
class EVTFitter:
    ...
    def _pickands(self, xs: np.ndarray) -> Tuple[float, float]:
        ...
        A = (x_nk - x_2k) / max(1e-12, (x_2k - x_4k))
        A = max(A, 1e-6)
        xi = 1.0 - (1.0 / math.log2(A))
...
class HTNRMemory:
    def __init__(self, embedder: "EmbedderProtocol", *,
        ...
        # ---- FIX: 명시적 초기화 (Numerics -> float 필드 전달) ----
        self.auto = AutoCal(
        ...
        # sentence embedding cache for MMR (문서 측면이므로 문서 임베딩 기준)
    ...
    # ---------- GPU OOC helpers ----------
    def _chunked_gpu_matvec_cached(self, host_pinned_t, device_vec, *, cover_ratio: float = 0.5) -> np.ndarray:
        import torch as _torch
    ...
    def _encode_cached_sentences(self, sents: List[str]) -> np.ndarray:
        if not sents:
            return np.zeros((0, self.dim), dtype=np.float32)
        # 문서 임베딩 경로 사용(요약/병합은 문서 사이드)
        misses = [s for s in sents if self._sent_cache.get("D:"+s) is None]
        if misses:
            ...
            for s, e in zip(misses, embs):
                self._sent_cache.put("D:"+s, e)
        out = []
        for s in sents:
            v = self._sent_cache.get("D:"+s)
            if v is None:
    ...
    def _ingest_batch_freeze(self, batch: List[BufferItem]) -> None:
        ...
        sims_to_ctx = E @ M_agg_0
        sims_to_ctx = np.clip(sims_to_ctx, -1.0, 1.0).astype(np.float32)
    ...
    def _interference_ann_density(self) -> None:
        ...
        if sims_all.size == 0:
            ...
            if total_pairs <= 0:
                return
            take = min(max_pairs, total_pairs)
            iu = np.triu_indices(L, k=1)
            m = iu[0].size
            if take < m:
                sel = self._rng.choice(m, size=take, replace=False)
                idx0 = iu[0][sel]; idx1 = iu[1][sel]
            else:
                idx0 = iu[0]; idx1 = iu[1]
            X = self._leaf_mat
        ...
        with self._lock:
            ...
            for n in self.nodes.values():
                ...
                if self.hyper.snapshot_store_sent_embs and n.sent_embs is not None:
                    row["sent_embs"] = n.sent_embs.tolist()
