...
class HTNRMemory:
    def __init__(self, embedder: "EmbedderProtocol", *,
        ...
        self._leaf_mat_gpu = None  # torch Tensor on device (optional)
        ...
        self._cond = threading.Condition(self._read_lock)
    ...
    def _chunked_gpu_matvec_cached(self, host_pinned_t, device_vec) -> np.ndarray:
        ...
        with self._ooc_lock:
            ...
            return out_np.astype(np.float32, copy=True)
    ...
    def _refresh_leaf_cache(self, *, mark_ann_dirty: bool = False, reuse_pinned: bool = True) -> None:
        with self._read_lock:
            while self._active_reads > 0 or self._writer_active:
    ...
    def _ensure_leaf_gpu_cache(self, *, prefer_half: bool = True, max_free_frac: float = 0.4) -> bool:
        """Ensure the leaf embedding matrix is cached on GPU for fast dot products."""
        if not (self._use_torch and self._device and getattr(self._device, "type", "") == "cuda"):
            self._leaf_mat_gpu = None
            return False
        if self._leaf_mat is None or not self.leaves:
            self._leaf_mat_gpu = None
            return False

        import torch as _torch

        target_shape = (len(self.leaves), self.dim)
        if self._leaf_mat_gpu is not None and tuple(self._leaf_mat_gpu.shape) == target_shape:
            return True

        if prefer_half and hasattr(_torch.cuda, "is_bf16_supported") and _torch.cuda.is_bf16_supported():
            dtype = _torch.bfloat16
            bytes_per = 2
        else:
            dtype = _torch.float16 if prefer_half else _torch.float32
            bytes_per = 2 if prefer_half else 4

        need_bytes = target_shape[0] * target_shape[1] * bytes_per
        try:
            free_b, _ = _torch.cuda.mem_get_info()
        except Exception:
            free_b = 0
        if need_bytes > free_b * max_free_frac:
            self._leaf_mat_gpu = None
            return False
        ...
        try:
            with self._read_lock:
                while self._writer_active:
                    self._cond.wait(timeout=0.01)
                if self._leaf_mat_host_t is not None and tuple(self._leaf_mat_host_t.shape) == target_shape:
                    src_cpu = self._leaf_mat_host_t
                else:
                    src_cpu = _torch.from_numpy(np.ascontiguousarray(self._leaf_mat)).pin_memory()
                self._leaf_mat_gpu = src_cpu.to(self._device, dtype=dtype, non_blocking=True)
            return True
        except Exception as e:
            logger.warning(f"Leaf GPU cache upload failed: {e}")
            self._leaf_mat_gpu = None
            return False
    ...
    def _batch_effective_node_embs(self, nodes: Sequence[HTNRNode], comp_vec: np.ndarray) -> np.ndarray:
        if not nodes:
            return np.zeros((0, self.dim), dtype=np.float32)
        embs = [self._effective_node_emb(node, comp_vec).astype(np.float32, copy=False) for node in nodes]
        return np.stack(embs, axis=0)

    def _batch_effective_node_embs_gpu(self, nodes: Sequence[HTNRNode], comp_vec: np.ndarray) -> np.ndarray:
        if not nodes:
            return np.zeros((0, self.dim), dtype=np.float32)
        if not (self._use_torch and self._device and getattr(self._device, "type", "") == "cuda"):
            return self._batch_effective_node_embs(nodes, comp_vec)
        ...
        for node in nodes:
            if self.hyper.branches_enable:
                try:
                    ...
                    pass
            E = None
            if self.hyper.branches_enable and node.branches is not None and node.branches.size > 0:
                E = node.branches
            if E is None:
                E = node.emb.reshape(1, -1)
            E = _normalize(E, 1e-8).astype(np.float32, copy=False)
            branch_list.append(E)
            branch_sizes.append(E.shape[0])

        Bmax = max(branch_sizes) if branch_sizes else 1
        D = self.dim
        device = self._device

        X = _torch.zeros((len(nodes), Bmax, D), dtype=_torch.float32, device=device)
        mask = _torch.zeros((len(nodes), Bmax), dtype=_torch.bool, device=device)
        for idx, E in enumerate(branch_list):
            if E.size == 0:
                continue
            t = _torch.from_numpy(np.ascontiguousarray(E)).to(device=device, dtype=_torch.float32)
            b = t.size(0)
            X[idx, :b, :] = t
            mask[idx, :b] = True

        comp_t = _torch.from_numpy(np.ascontiguousarray(comp_vec)).to(device=device, dtype=_torch.float32)
        M_t = _torch.from_numpy(np.ascontiguousarray(self.M_agg)).to(device=device, dtype=_torch.float32)

        s_q = _torch.einsum("nbd,d->nb", X, comp_t)
        s_m = _torch.einsum("nbd,d->nb", X, M_t)
        kq = float(self.hyper.branch_gate_kappa_q)
        km = float(self.hyper.branch_gate_kappa_m)
        score = kq * s_q + km * s_m
        score = _torch.where(mask, score, _torch.full_like(score, -1e9))

        ex = _torch.exp(score - score.max(dim=1, keepdim=True).values)
        g = ex / (ex.sum(dim=1, keepdim=True) + 1e-8)

        if (self.hyper.branch_phi_nonlinear or "tanh") == "tanh":
            phi = _torch.tanh(2.0 * s_q)
        else:
            phi = _torch.zeros_like(s_q)

        w = g * (1.0 + 0.2 * phi)
        w = _torch.where(mask, w, _torch.zeros_like(w))
        w = w / (w.sum(dim=1, keepdim=True) + 1e-8)

        eff = _torch.einsum("nb,nbd->nd", w, X)
        eff = _torch.nn.functional.normalize(eff, dim=1, eps=1e-8)

        w_cpu = w.detach().cpu().numpy().astype(np.float32)
        for node, weights, b in zip(nodes, w_cpu, branch_sizes):
            if node.b_sali is not None and node.b_sali.size >= b:
                node.b_sali = 0.9 * node.b_sali + 0.1 * weights[:b]

        return eff.detach().cpu().numpy().astype(np.float32)
    ...
    def retrieve_for_query(self, query: str, *, K_cap: int = 8, mutate: bool = False,
        ...
        try:
            def _do_retrieve() -> List[Any]:
                ...
                leaf_mat_local = np.asarray(leaf_mat, dtype=np.float32)
                ...
                if self._use_torch and self._device and self._device.type == "cuda":
                    use_gpu_mat = self._leaf_mat_gpu is not None or self._ensure_leaf_gpu_cache()
                    if use_gpu_mat and self._leaf_mat_gpu is not None:
                        comp_cpu = torch.from_numpy(np.ascontiguousarray(comp)).pin_memory()
                        comp_t = comp_cpu.to(self._device, dtype=self._leaf_mat_gpu.dtype, non_blocking=True)
                        scores_t = torch.matmul(self._leaf_mat_gpu, comp_t)
                        ...
                        if self._leaf_mat_host_t is not None:
                            try:
                                ...
                                scores = (leaf_mat_local @ comp).astype(np.float32)
                        else:
                            scores = (leaf_mat_local @ comp).astype(np.float32)
                else:
                    scores = (leaf_mat_local @ comp).astype(np.float32)
                ...
                idx0 = _topk_indices(scores, k0)
                node_ids0: List[int] = []
                nodes0: List[HTNRNode] = []
                for ii in idx0:
                    nid = leaf_ids[ii]
                    node = self.nodes.get(nid)
                    if not node:
                    ...
                    if self.hyper.branches_enable:
                        try:
                            ...
                            pass
                    node_ids0.append(nid)
                    nodes0.append(node)

                if self._use_torch and self._device and self._device.type == "cuda":
                    effs = self._batch_effective_node_embs_gpu(nodes0, comp)
                else:
                    effs = self._batch_effective_node_embs(nodes0, comp)

                if effs.size == 0:
                    top_pairs: List[Tuple[float, int]] = []
                    chosen_ids: List[int] = []
                    score_map: Dict[int, float] = {}
                else:
                    base_sc = (effs @ comp).astype(np.float32)
                    vr = np.array([
                        max(-1.0, min(1.0, node.V * node.R / 2.0)) for node in nodes0
                    ], dtype=np.float32)
                    sc = base_sc * (1.0 + 0.25 * vr)
                    order = np.argsort(-sc)[:K_cap]
                    top_pairs = [(float(sc[i]), node_ids0[i]) for i in order]
                    chosen_ids = [node_ids0[i] for i in order]
                    score_map = {nid: float(score) for score, nid in top_pairs}
    ...
    def debug_dma_state(self) -> Dict[str, Any]:
        return {
            "torch_available": _HAS_TORCH,
            "cuda_available": _TORCH_CUDA,
            "use_torch": self._use_torch,
            "device": str(self._device) if self._device else None,
            "leaf_mat_shape": tuple(self._leaf_mat.shape) if self._leaf_mat is not None else None,
            "leaf_gpu_cached": self._leaf_mat_gpu is not None,
            "leaf_gpu_dtype": str(self._leaf_mat_gpu.dtype) if self._leaf_mat_gpu is not None else None,
            "have_pinned_host": self._leaf_mat_host_t is not None,
            "ooc_chunk_rows": self._ooc.get("chunk_rows"),
            "ooc_buf_rows": self._ooc.get("buf_rows"),
            "active_reads": self._active_reads,
            "writer_active": getattr(self, "_writer_active", False),
        }
